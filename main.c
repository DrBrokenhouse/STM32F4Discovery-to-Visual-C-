/*
	...
	Version 3.1 Interrupt used to echo serial back to Visual Studio C#
	----------------------------------------------------------
	Special notes:
		...
		while(LED_Flag = 1) //this flag is set in UART4_IRQHandler()
		{
			...
			if((LED_Command >= 22201) || (LED_Command <= 22214))
			{
				...
				case 22201:// Blink on ALL Led
					...
					AllBlink(25,100,25); //number of blinks, time on, time off
					beep(3,100,25);
					LED_Status_Reset();
				...
				case 22206: // Turns on the green LED
					...
					beep(1,75,0); // 1 loop, 500ms on with 75ms delay off
					LED_Status_Reset();
...
void SelfTest(void)
{
	...
	while(rounds)
	{
		...
		beep(1,25,25);  // 1 loop, 100ms on with 75ms delay off
		Delay(25);// set to 150ms
		//LEDs off
		GPIO_ResetBits(GPIOD, GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15);
		Delay(25);// set to 1500ms
		rounds--;
	}
	USART_puts(UART4, "\n");
	LED_Status_Reset();
...
void pinwheel(void)
{
	int round = 25; //This sets the number of revolutions and the speed.
	while(round)
	{
		USART_puts(UART4, ".");
		GPIO_SetBits(GPIOD, GPIO_Pin_12);
